...
Ports-and-Adapters será usado **onde houver um seam real**, e não como obrigação de criar interfaces para todas as dependências.

A regra de design é:

```text
necessidade de substituição / isolamento
+
benefício de teste / variação
+
responsabilidade que merece um seam
        ↓
PORTA / INTERFACE
        ↓
ADAPTERS
...
O princípio de referência para este projeto é:

> **Uma implementação isolada não justifica automaticamente um port. Duas implementações justificadas revelam um seam real.**

Exceções são permitidas quando a abstração melhora claramente a testabilidade, o isolamento de um detalhe externo ou a estabilidade do contrato, mas devem ser justificadas.

### 11.1 Input Ports

Input ports representam operações que o sistema expõe quando uma fronteira externa realmente existir, por exemplo:
...
Esses nomes representam casos de uso; não implicam automaticamente interfaces formais.

### 11.2 Output Ports

Output ports representam dependências externas ou substituíveis que merecem um seam, por exemplo:

```text
ProjectRepository      ← seam provável
AgentRuntime            ← seam real
ExternalEvidenceSource  ← seam conforme integração
TelemetrySink           ← seam conforme uso
```

Catálogos e dependências locais não devem virar ports apenas por antecipação.
...
# 39. Repository Abstractions and Seams

Repositories serão definidos como abstrações apenas quando houver uma necessidade real de separar domínio/aplicação do mecanismo de persistência ou de permitir substituição/isolamento relevante.
...
Mas cada um deverá passar por uma avaliação de seam antes da implementação.

### Critérios

```text
Há dependência externa relevante?
Há necessidade de substituição?
Existe implementação de produção + stand-in de teste?
A interface reduz complexidade para o chamador?
```

Se a resposta for predominantemente não, preferir uma implementação local mais simples e adiar o seam.

Repositories devem permanecer responsáveis por persistir, recuperar e consultar, e não por regras de negócio, seleção de modelos, construção de prompts ou execução de agentes.

---

# 40. Runtime Port

`AgentRuntime` representa exclusivamente a execução de agentes através de um runtime. Descoberta de recursos e telemetria não fazem parte automaticamente deste contrato; pertencem a capacidades do ecossistema e da observabilidade.

Interface conceitual candidata:
...
A implementação concreta será OpenClaw, Hermes ou outra. O contrato definitivo será reduzido ao menor conjunto de operações necessário ao primeiro vertical slice.

---

# 41. Model Catalog Abstraction

`ModelCatalog` representa conhecimento sobre modelos elegíveis. Ele não será automaticamente tratado como um output port.

Na primeira implementação, o catálogo poderá ser um módulo interno/in-memory. Um adapter externo será introduzido quando a descoberta de modelos, preços ou disponibilidade depender de um sistema externo real.
...
A interface definitiva será escolhida após a análise do primeiro caso de uso real.

---

# 42. Skill Catalog Abstraction

`SkillCatalog` representa o inventário de Skills e suas propriedades relevantes.

Na primeira implementação, deve ser preferencialmente um módulo local. Ele só ganhará um adapter externo se existir uma fonte externa real de descoberta/registro que justifique um seam.

Além disso, o catálogo deverá futuramente representar o modelo de Skills consolidado a partir das referências estudadas, incluindo, quando aplicável:
...
# 43. Agent Catalog Abstraction

`AgentCatalog` representa o inventário de agentes elegíveis.

Na primeira implementação, pode permanecer in-process. O seam externo será introduzido quando a descoberta de agentes depender de um runtime ou registry externo real.
...
Tempo é uma dependência transversal, mas não precisa automaticamente de um port público.

A regra preferida é:

```text
caso de uso / domínio
→ Clock interno simples ou dependência injetável
```

Um seam explícito só será criado se houver necessidade real de controlar tempo, múltiplas implementações relevantes ou forte benefício de teste.

---

# 47. Identifier Generation Dependency

Geração de identificadores segue a mesma disciplina.

Não criar um `IdGenerator` como abstração pública apenas porque testes podem utilizar valores determinísticos. Uma função/factory simples pode ser suficiente.

Um seam explícito é justificável quando existir uma política relevante de geração, múltiplas estratégias ou integração externa que precise ser isolada.
...
Uma abstração conceitual **candidata**, a validar pelo primeiro vertical slice de raciocínio do Orchestrator:
...
# 242. Deep Module and Seam Discipline

O Design incorpora a disciplina de **módulos profundos** fornecida pela referência `codebase-design`.

### Princípio DM-001 — Profundidade

Um módulo deve esconder quantidade significativa de comportamento atrás de uma interface proporcionalmente pequena.

A avaliação de um módulo deve considerar:
...
Não usar quantidade de linhas como medida de profundidade.

### Princípio DM-002 — Interface é a superfície de teste

Testes de comportamento devem preferencialmente atravessar a interface externa do módulo. Se for necessário testar frequentemente o interior do módulo, isso é um sinal para revisar a forma do módulo ou identificar um seam interno privado.

### Princípio DM-003 — Deletion test

Para cada módulo relevante, perguntar:

```text
Se este módulo fosse removido,
a complexidade desapareceria
ou reapareceria distribuída entre os chamadores?
```

Se a complexidade simplesmente reaparecer nos chamadores, o módulo provavelmente possui profundidade e leverage reais. Se nada importante mudar, o módulo pode estar apenas retransmitindo chamadas.

### Princípio DM-004 — Não criar seams especulativos

Uma interface/port deve possuir uma justificativa real. O simples fato de uma segunda implementação ser teoricamente possível não é suficiente.

### Princípio DM-005 — Adapters são papel arquitetural

Adapter descreve a implementação concreta que ocupa um seam. Não representa necessariamente uma tecnologia, uma classe pequena ou uma integração remota.

### Princípio DM-006 — Teste por substituição, não por camadas artificiais

Quando um módulo for aprofundado, testes do comportamento antigo podem ser substituídos por testes na interface do módulo aprofundado quando forem redundantes. Não manter camadas de testes apenas por tradição.

---

# 243. Dependency Category for Seam Decisions

Ao decidir se uma dependência merece um seam, classificá-la como:
...
Preferir composição interna sem port externo.

### Local-substitutable

Usar stand-in local quando ele existir e for relevante ao teste; o seam pode permanecer interno ao módulo.

### Remote but owned

Quando existir uma fronteira de rede sob nosso controle, um port pode ser apropriado e adapters de produção/teste podem ocupar o seam.

### True external

OpenClaw, Hermes, providers de modelos e serviços externos são candidatos naturais a seams, desde que o contrato interno seja realmente mais estável e útil que a dependência externa.

---

# 244. Critical Module Review

Os seguintes módulos receberam prioridade de revisão de profundidade por concentrarem decisões importantes:
...
Deve esconder seleção multicritério, compatibilidade, policy, custo, risco e evidência atrás de uma interface simples. O chamador deve expressar a necessidade de seleção, não reproduzir o algoritmo de seleção.

**Risco identificado:** fragmentar a lógica entre `CapabilityResolver`, `AgentCandidates`, `SkillCandidates`, `ModelCandidates`, `PolicyFilter` e `MultiObjectiveEvaluator` pode produzir módulos rasos. Esses elementos são candidatos a componentes internos da implementação de `ResourceSelection`, não necessariamente interfaces públicas.

### Delegation

Deve encapsular preparação e execução do handoff atrás de uma interface pequena. O seam real está principalmente entre nosso modelo interno e runtimes externos.

**Decisão:** `AgentRuntime` continua como seam arquitetural válido porque a direção do projeto exige, no mínimo, adaptação ao OpenClaw e possibilidade de Hermes/outro runtime.

### Result Evaluation

Deve esconder composição de critérios, evidências, avaliação e veredito. `CriteriaResolver`, `EvidenceResolver` e demais auxiliares devem ser tratados como seams internos até que uma necessidade independente os justifique.

### Replanning

É um forte candidato a módulo profundo. Impact analysis, affected-set resolution, dependency propagation, resource reassessment e plan validation devem ficar atrás de uma interface de alto nível sempre que isso preservar locality.

### Context / Knowledge Access

O sistema precisa distinguir `Knowledge` de `Context`, mas não deve criar automaticamente um serviço/port para cada mecanismo de retrieval. O ponto de desenho deve ser o comportamento de seleção e obtenção de contexto, com detalhes de armazenamento/retrieval ocultos atrás do menor seam necessário.

### Agent Runtime

É um seam real. A implementação inicial será OpenClaw; a arquitetura deve permitir outro runtime sem contaminar Application/Domain. A interface deve ser cuidadosamente reduzida para não reproduzir a API do runtime.

---

# 245. Design It Twice — Interface Candidates

Para os módulos críticos, a primeira interface não será tratada como definitiva. Serão consideradas alternativas radicalmente diferentes e comparadas por **depth, leverage, locality e seam placement**.

## 245.1 Resource Selection

### Opção A — comando mínimo

```text
select(workUnit, context) → ResourceConfiguration
```

**Força:** interface muito pequena e alto leverage.

**Risco:** pode ocultar decisões demais se o contrato não explicar critérios, erros e políticas observáveis.

### Opção B — pipeline explícito
...
**Força:** flexibilidade e observabilidade.

**Risco:** expõe demais da implementação ao chamador.

### Opção C — policy-driven selection

```text
select(requirement, policy, evidence) → SelectionDecision
```

**Força:** explicita a natureza decisória e favorece rastreabilidade.

**Risco:** pode deslocar demais o conhecimento específico da Work Unit para fora da seleção.

**Recomendação:** usar uma forma próxima da **Opção A**, mas retornando uma decisão rica o suficiente para auditoria. A pipeline interna permanece privada.

---

## 245.2 Delegation

### Opção A — submit package

```text
execute(TaskPackage) → ExecutionReference
```

### Opção B — lifecycle explícito
...
### Opção C — capability-oriented execution

```text
run(workUnit, resourceConfiguration) → ResultPackage
```

**Recomendação:** preferir uma interface de alto nível próxima da **Opção C** no Application layer, com um contrato runtime interno equivalente à **Opção A/B** dentro do adapter. Isso preserva profundidade no Application e não obriga o Domain a conhecer o lifecycle específico do runtime.

---

## 245.3 Result Evaluation

### Opção A — evaluate result

```text
evaluate(result, criteria, context) → Evaluation
```

### Opção B — staged evaluation
...
### Opção C — evaluation request

```text
evaluate(EvaluationRequest) → Verdict
```

**Recomendação:** **Opção A/C híbrida**: uma entrada única `EvaluationRequest` e uma resposta rica `Evaluation`, mantendo pipeline interna privada.

---

## 245.4 Replanning

### Opção A — replan command

```text
replan(projectState, trigger) → PlanRevision
```

### Opção B — staged planning pipeline
...
### Opção C — change-directed planning

```text
apply(change) → UpdatedPlan
```

**Recomendação:** **Opção A**, deixando as etapas de impacto, propagação e validação internas. O chamador precisa pedir replanejamento; não precisa conhecer o algoritmo.

---

# 246. Deepening Decisions

Com base na revisão, as seguintes decisões substituem interpretações excessivamente estruturais do design v0.1:

1. `Resource Selection`, `Result Evaluation` e `Replanning` serão tratados como candidatos a **módulos profundos**, com interfaces pequenas e implementação interna composta.
2. `AgentRuntime` é um **seam real** e manterá um adapter explícito.
3. `ModelCatalog`, `SkillCatalog` e `AgentCatalog` não serão ports externos por padrão; começarão como módulos in-process e ganharão adapters somente diante de integração real.
4. `Clock` e `IdGenerator` não serão abstrações públicas por padrão; serão dependências internas simples até existir razão para um seam.
5. `CriteriaResolver`, `EvidenceResolver`, `CapabilityResolver`, `AffectedSetResolver` e elementos equivalentes serão tratados como **seams internos potenciais**, não como interfaces públicas automáticas.
6. O número de interfaces públicas deve ser minimizado pelo princípio de depth.
7. O teste de comportamento deverá ocorrer preferencialmente na interface externa do módulo.
8. Ao aprofundar um módulo, testes redundantes dos módulos rasos anteriores poderão ser removidos.
9. `AgentRuntime` não deverá misturar execução, descoberta de recursos e telemetria em uma interface única; essas capacidades pertencem a seams diferentes.

---

# 247. Skill Architecture Implications

A referência de Skills estudada acrescenta requisitos importantes ao design de `SkillProfile`. O modelo atual deve evoluir futuramente para representar, quando aplicável:
...
A forma física final será definida no documento específico de arquitetura de Skills. Não transformar esta observação em código ainda.

---

# 248. Domain Context and Ubiquitous Language

O projeto deverá possuir um `CONTEXT.md` próprio quando a linguagem canônica estiver suficientemente consolidada. Seu papel será:

```text
definir o que um conceito é
fixar o termo canônico
registrar termos a evitar
``
...
A criação deve ser incremental, conforme termos forem resolvidos.

---

# 249. Revised Architecture Quality Gate

Antes da implementação, cada módulo crítico deverá ser capaz de responder:

```text
1. Qual é sua responsabilidade principal?
2. Qual complexidade ele esconde?
3. Qual é sua interface real?
4. Qual é o seu seam, se houver?
5. Existem adapters reais?
6. O módulo possui depth e leverage?
7. Há locality para manutenção?
8. O teste pode atravessar a interface?
9. Existe alguma abstração especulativa?
10. O design respeita SDD e rastreabilidade?
```

Se essas respostas não forem satisfatórias, o módulo não deve ser considerado pronto para implementação.

---

# 250. Specification-Driven Development
...
**Status:** Design conceitual revisado — v0.2.

A revisão acrescentou disciplina de módulos profundos, seam discipline, critérios de abstração e revisão de interfaces críticas antes da implementação.
...
Antes do primeiro código, o projeto deverá transformar o Design revisado em Work Units de implementação ordenadas por dependência, com critérios de aceitação e estratégia de verificação. Isso é requisito do SDD e evita iniciar a implementação com seams ainda indefinidos.
