...
A sequência exata de artefatos e etapas será definida nas etapas posteriores da especificação.
...
# 23. Dependências Parciais

Uma unidade pode estar:

### LIBERADA

Todas as dependências necessárias estão validadas.

### PARCIALMENTE LIBERADA

Parte do trabalho pode avançar com segurança.

### BLOQUEADA

Não há base suficiente para avançar.
...
`decisão → planejamento da alteração → execução → teste → validação → análise de efeitos → conclusão`
...
# 26. Estados de Execução

Uma unidade de trabalho pode assumir estados como:
...
# 30. Questões Ainda em Aberto

Os itens abaixo ainda não estão definidos nesta versão:

1. Estrutura canônica definitiva da espinha dorsal documental.
2. Critérios formais completos de conclusão de cada grande etapa.
3. Critérios formais completos para cada tipo de validação.
4. Estrutura formal do estado operacional do projeto.
5. Estrutura formal do grafo de conhecimento e rastreabilidade.
6. Política detalhada de replanejamento médio.
7. Algoritmo detalhado de priorização da fila de trabalho.
8. Política formal de versionamento de baselines.
9. Estrutura definitiva de avaliação/evals da Skill.
10. Política detalhada de atualização automática do conhecimento.
11. Estrutura definitiva de `SKILL.md`, `references/`, `workflows/`, `prompts/`, `assets/`, `scripts/` e `evals/`.

---

# 31. Status da Especificação
...
# 32. Regra de Governança deste Documento
