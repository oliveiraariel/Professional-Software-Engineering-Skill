...
# 17. Etapa 4 — Necessidades e Requisitos

A Etapa 4 transforma a compreensão do problema em uma definição suficientemente formal do que o projeto precisa atender, preservando a separação entre problema, necessidade, requisito, regra, restrição, critério de aceitação e solução.

A etapa deve produzir uma especificação suficientemente clara, consistente, rastreável e verificável para permitir o avanço responsável para as etapas seguintes, sem antecipar decisões de domínio, dados, arquitetura ou implementação.

## 17.1 — Compreensão e Contexto do Problema

O agente deve identificar, conforme aplicável:

- problema;
- contexto;
- situação atual;
- oportunidade, quando existir;
- objetivos conhecidos;
- stakeholders relevantes;
- restrições conhecidas.

**Problema** representa a situação indesejada, limitação, dificuldade ou necessidade que justifica o projeto.

**Contexto** representa as condições nas quais o problema existe.

**Situação atual** descreve como o processo ou ambiente funciona atualmente.

**Oportunidade** representa uma possibilidade de melhoria, crescimento, automação, inovação ou outro resultado desejável, quando existir.

**Objetivo** representa aquilo que o projeto pretende alcançar.

**Stakeholder** representa uma parte com interesse, responsabilidade, influência ou que seja afetada pelo projeto.

**Restrição** representa uma condição conhecida que limita a execução ou as alternativas do projeto.

As informações devem manter, quando necessário, seu estado epistemológico:
...
O agente não deve transformar ausência de informação em fato.

### Critério de suficiência

O bloco está suficientemente definido quando for possível responder:
...
quem é relevante?
quais objetivos já são conhecidos?
quais restrições já são conhecidas?
```

## 17.2 — Objetivos e Resultados Esperados

Este bloco formaliza os objetivos identificados no contexto.

```text
OBJETIVO
├── resultado esperado
├── indicador, quando aplicável
└── critério de sucesso, quando aplicável
```

**Objetivo** descreve aquilo que o projeto pretende alcançar, mudar, preservar ou obter.

**Resultado esperado** representa o estado ou resultado observável que se espera alcançar em função do objetivo.

**Indicador** representa uma medida utilizada para acompanhar ou verificar o resultado quando sua utilização for pertinente.

**Critério de sucesso** define a condição que permite determinar se o objetivo ou resultado esperado foi suficientemente alcançado.

Indicadores e metas não são obrigatórios para todo objetivo. Devem existir quando forem relevantes e houver base suficiente para defini-los.

O agente não deve inventar indicadores, valores ou metas.

```text
OBJETIVO ≠ SOLUÇÃO
```

A identificação inicial de objetivos em 17.1 deve ser tratada como informação contextual; sua formalização ocorre neste bloco.

## 17.3 — Stakeholders e Necessidades

Este bloco identifica os stakeholders e estrutura seus interesses, necessidades e expectativas para posterior derivação de requisitos.
...
**Stakeholder** é qualquer parte relevante para o projeto por possuir interesse, responsabilidade, influência ou por ser afetada por seus resultados.

```text
STAKEHOLDER ≠ ATOR
```

O conceito de ator será tratado posteriormente no contexto das interações com o sistema.

**Necessidade** representa aquilo que uma parte precisa obter, resolver, preservar ou melhorar.

**Interesse** representa aquilo que é importante para o stakeholder.

**Expectativa** representa aquilo que o stakeholder espera obter como resultado ou comportamento.

```text
NECESSIDADE
→ situação ou capacidade que precisa ser atendida

REQUISITO
→ definição formal do que deverá ser atendido
```
...
Não deve existir transformação mecânica de uma necessidade em um único requisito.

## 17.4 — Escopo e Limites

O escopo estabelece as fronteiras do projeto.
...
**Escopo** define aquilo que pertence ao projeto.

**Fora do escopo** define aquilo que não pertence ao projeto.

**Limites** definem as fronteiras de responsabilidade e atuação do projeto.

**Premissas** representam condições assumidas como válidas para permitir o planejamento, mas que podem exigir confirmação posterior.

**Dependências externas** representam elementos externos dos quais o projeto depende e sobre os quais não possui controle completo.
...
Uma alteração relevante de escopo deve seguir as regras de análise de impacto definidas pela Skill.

## 17.5 — Requisitos Funcionais

Requisito funcional é a especificação formal de um comportamento, capacidade, processamento ou resultado que o sistema deve fornecer para atender uma necessidade ou outra fonte legítima de obrigação do projeto.

Exemplos de comportamentos podem envolver:
...
Um requisito funcional deve ser, quando aplicável:
...
Um requisito pode possuir origem em:
...
- legislação;
- objetivo;
- outra obrigação legítima do projeto.

Sua origem deve ser rastreável.

Requisitos conflitantes devem ser identificados e tratados segundo as regras de evidência, impacto, autoridade e intervenção humana.

## 17.6 — Requisitos Não Funcionais

Requisito não funcional especifica uma propriedade, qualidade, condição de operação ou restrição relevante do sistema.

Categorias recorrentes podem incluir:
...
Essas categorias são adaptativas e não constituem checklist obrigatório.

Um RNF pode decorrer de:
...
- objetivo;
- condição necessária do serviço.

RNFs devem possuir, quando aplicável, critérios suficientemente definidos para permitir sua verificação.

O agente não deve inventar metas, limites ou valores.

```text
RNF ≠ RESTRIÇÃO TÉCNICA
```

Uma restrição técnica pode existir sem ser classificada como RNF.

Uma representação conceitual de um RNF pode conter:
...
## 17.7 — Regras de Negócio e Restrições
...
**Regra de negócio** representa uma condição, política, princípio ou lógica pertencente ao domínio ou à organização e que influencia o comportamento do projeto.

**Requisito** representa aquilo que o sistema precisa atender.

**Restrição** representa uma condição que limita as alternativas ou a execução do projeto.

Uma regra pode gerar múltiplos elementos:
...
Uma restrição pode influenciar uma decisão de solução:
...
As relações devem ser rastreáveis.

Conflitos entre regras, requisitos ou restrições devem ser identificados e tratados segundo as regras de evidência, impacto, autoridade e intervenção humana da Skill.

O agente não deve escolher silenciosamente uma interpretação quando a decisão exigir autoridade ou participação humana.

## 17.8 — Critérios de Aceitação

Critério de aceitação é a condição objetiva e verificável utilizada para determinar se um requisito foi atendido.
...
CRITÉRIO DE ACEITAÇÃO
→ condição para considerar atendido

TESTE
→ meio de verificar
```

Critérios devem ser adequados à natureza do requisito.

Podem ser quantitativos ou qualitativos, desde que permitam avaliação objetiva suficiente ao contexto.

O agente não deve inventar metas ou condições sem evidência ou decisão legítima.

O método de teste não precisa necessariamente ser definido nesta etapa.

A cadeia de rastreabilidade pode ser:
...
A validação posterior pode utilizar os critérios para determinar:

```text
todos os critérios aplicáveis atendidos
→ APROVADA

critério relevante não atendido
→ REPROVADA
```

## 17.9 — Rastreabilidade

A rastreabilidade deve permitir compreender a origem, o motivo, as dependências, os impactos e a verificação dos elementos relevantes.

A cadeia principal da Etapa 4 é:
...
As relações podem ser transversais:

```text
Regra de Negócio → Requisito
Restrição → Requisito / Decisão de Solução
Legislação → Regra / Restrição / Requisito
Decisão → Elementos Afetados
Mudança → Impactos
```

A rastreabilidade deve permitir, quando necessário:
...
O nível de rastreabilidade deve ser proporcional à:
...
A implementação física da rastreabilidade pode variar conforme o projeto.

## 17.10 — Validação da Especificação

A própria especificação de necessidades e requisitos deve ser validada antes de a Etapa 4 ser considerada concluída.
...
coerência com necessidades
coerência com escopo
coerência com regras e restrições
```

### Validação de completude

As necessidades relevantes devem ter sido analisadas e os requisitos correspondentes identificados quando aplicável.

### Validação de consistência

Devem ser identificados conflitos entre:
...
ambiguidades
termos indefinidos
generalidades excessivas
interpretações múltiplas
```

### Validação de rastreabilidade

Deve ser possível acompanhar as relações relevantes entre:
...
### Validação de verificabilidade

Cada requisito relevante deve possuir uma forma razoável de determinar posteriormente se foi atendido.

A existência do teste executável não é obrigatória nesta etapa.

### Validação de escopo

Todo requisito deve possuir relação coerente com o escopo definido.
...
`BLOQUEADA` é estado operacional e não resultado de validação.

#### APROVADA

A especificação está suficientemente madura para permitir avanço responsável.

#### REPROVADA

Existem problemas que precisam ser tratados.
...
Se a continuidade não puder ser resolvida autonomamente ou exigir decisão crítica:
...
A Etapa 4 pode ser considerada concluída quando:

```text
objetivos relevantes compreendidos
+
necessidades relevantes identificadas
+
escopo definido
+
requisitos formalizados
+
regras e restrições tratadas
+
critérios de aceitação definidos quando necessários
+
...
A Etapa 4 não precisa concluir:
...
Restrições, decisões ou informações técnicas já existentes podem ser registradas quando forem fatos ou condicionantes do projeto, sem antecipar a modelagem da solução.
...
# 24. Dependências e Disponibilidade
...
# 26. Análise de Impacto de Mudança
...
# 28. Baseline, Conclusão e Reabertura
...
# 30. Estado Operacional do Projeto
...
# 31. Rastreabilidade e Grafo de Conhecimento
...
# 35. Questões Remanescentes da Implementação

Os princípios conceituais consolidados até a Etapa 4 estão definidos.
...
Esses elementos devem ser definidos posteriormente de forma coerente com esta especificação e não constituem pendências conceituais das etapas já consolidadas.

---

# 36. Status da Especificação
...
# 37. Regra de Governança deste Documento
